
\begin{figure*}
  \begin{center}
    \subfloat[Part A][\label{fig:addA}Both Process~$a$ and Process~$d$
      become sources.  $w_{ab} = 2$; $w_{bc} = w_{bd} = 1$; $w_{cd} =
      3$.]  {\input{input/figaddA.tex}}
    \hspace{5pt}
    \subfloat[Part B][\label{fig:addB}Messages transit through communication
      links and carry increasing weights.]{\input{input/figaddB.tex}}
    \hspace{5pt} \subfloat[Part C][\label{fig:addC}Process~$b$ and
      Process~$c$ receive, deliver, and forward $\alpha_{d}^{1}$ and
      $\alpha_d^3$ respectively.]{\input{input/figaddC.tex}}
    \hspace{5pt} \subfloat[Part D][\label{fig:addD}$a$ and
      $b$ discarded their received messages. $c$ still improved with $\alpha_d^2$.]
    {\input{input/figaddD.tex}}
    \caption{\label{fig:add}Efficient consistent partitioning using
      \NAMEB. Partition~$P_a$ includes $a$ while Partition~$P_d$
      includes $b$, $c$, and $d$. Process~$c$ and Process~$d$ never
      acknowledge the existence of Source~$a$, for Process~$b$ stops
      the propagation of the latter's notifications.}
  \end{center}
\end{figure*}

%%% Local Variables: 
%%% mode: latex
%%% TeX-master: "../paper"
%%% ispell-local-dictionary: "english"
%%% End: 

