...
\end{inparaenum}
\note{Je ne vois pas l'interet des enums ici? ca casse la lecture}
\item [Location:]
...
carry. We call such a primitive \emph{scoped broadcast}
\note{connerie: ca marche pas comme ca la propagation des routes dans les routeurs?On est pas entrain de reinventer une algo qui existe depuis bien longtemps dans la communauté reseau? Si ce n'est pas le cas alors nous avons une autre application pour as-cast}
(see
...
would\note{can?} block the propagation of other partitions (see
