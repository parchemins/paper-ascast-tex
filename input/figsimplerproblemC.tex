...

  ...
  \draw (-\X + 5pt, 0) --
  node[above=-0.3em, font=\tiny]{~ ~ ~ ~ ~ $\DEL_{a} \rightarrow$} %% b - a
  node[above=-0.3em, font=\tiny]{~ ~ ~ ~ ~ $\textcolor{\WRONG}{\text{\normalsize\xmark}} \hphantom{\rightarrow}$} %% b - a
  node[below=-0.3em, font=\tiny]{$\leftarrow \textcolor{\PC}{\ADD_c^3}$} %% b - a
  (0 - 5pt, 0);

  \draw (0 +5pt, 0) --
  node[below=-0.3em, font=\tiny]{$\leftarrow \DEL_c \vphantom{\ADD^1}$}
  (\X -5pt, 0); %% b - c
