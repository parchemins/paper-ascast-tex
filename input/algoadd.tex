...
\Function{\textup{\texttt{Add ( )}} \tcp*[f]{generates notification message: $\alpha_p^0$}} {
...
\Function{\textup{receiveAdd($q$, $s'$, $d'$)} \tcp*[f]{$r_p(\alpha_{s'}^{d'})$ from $q$}} {
  \If (\tcp*[f]{distance predicate ($\phi$)}){$d' < d$} {
      ...
      \ForEach(\tcp*[f]{\smash{forward to neighbors $f_p(\alpha_{s'}^{d'})$}}) {$n \in O_p \setminus q$} {
          \textup{send$_n$($s', d' + W_{pn}$)} \label{line:accumulator}
          \tcp*[r]{with updated metadata ($\mu$) \smash{$s_{pn}(\alpha_{s'}^{d'+w_{pn}})$}}
       }      
  }
}
