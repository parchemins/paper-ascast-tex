...
\underline{t}ables (DHT)~\cite{maymounkov2002kademlia} spread across
the different \processes composing the infrastructure. When a client
wants to access specific content, it requests a remote \process to
provide at least one \process identity to retrieve this content
...
These approaches contradict with the objectives of Edge
...
\noindent The contribution of this paper is threefold:
...
\item [$\bullet$] We provide an implementation entitled \NAME that
  ...
  to creations and deletions of partitions in the system. \NAME's
...
\item [$\bullet$] We evaluate \NAME with simulations. Our experiments
  ...
  \NAME in large scale networks comprising 10k \processes. Our
  experiments also highlight the relevance of \NAME in autonomous
  systems. \NAME would allow to lock down the traffic generated by
  content indexing, even in contexts where \emph{physical}
...
\noindent \NAME can constitute a new building block for
