...
\item [$\bullet$] We provide an implementation entitled \NAME that
  uses aforementioned principles to \underline{a}dapt its
  partitioning, \ie \underline{s}copes, to creations and deletions of
  partitions in the system. \NAME relies on a communication primitive
  \underline{s}coped-broad\underline{cast} as \TODO{a communication
    primitive that allows epidemic dissemination of messages as long
    as receiving \processes verify an application-dependant
    predicate.}
