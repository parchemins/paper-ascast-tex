...
To mitigate the broadcasting overhead, a \process creating a replica should notify all and only
...
partition.
% TODO: A periodic protocol... why a periodic protocol solves the ''correct cast/scope''? I miss the point here 
\textcolor{red}{
A periodic advertisement protocol already provides both
...
\item [$\bullet$] We evaluate \NAME through simulations using
  PeerSim~\cite{montresor2009peersim}. Results show the consistent
  partitioning of the algorithm as well as its scalability. Using
  scoped broadcast, \NAME quickly disseminate messages to a subset of
  relevant \processes. The higher the number of partitions, the
  smaller \NAME's overhead.  Most importantly, \NAME does not generate
  traffic in quiescent systems.  % Our experiments
