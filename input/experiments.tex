...
traffic to reach consistent partitioning.\note{Pour moi ce n'est pas assez claire pourquoi more traffic to reach CP +proposition de reformulation voir tex}
% Weight and latency are different in order to imply more concurrent operations (hence more traffic to reach consistent partitioning): the first $\alpha$ received by a node may not be that of its shortest path. 
...
\noindent Therefore, the first partition is the most expensive, for $\alpha$'s\note{'s pas compris? all $alpha$ sounds better?}
must reach every \process which takes time and generate more traffic.
\NAME quickly converges towards consistent partitioning in only\note{in only... why only? compare to what ? Is it good bad? why?}
% we should give additional elements to explain why it is rather ok to converge in 350ms: maybe by underlining the nb of nodes and the average latency between nodes.
...
\begin{figure*}
  \subfloat[An infrastructure composed of 40 interconnected autonomous systems and
  ...
  \hspace{5pt}
  \subfloat[Traffic overhead and indexes
    maintained by \NAME and its extension \NAMEC]{
...
\begin{asparadesc}
  ...
  \item[Description:] We build a network of networks by using another
    ...
    Europe~\cite{knight2011internet} \note{Ici c'est Géant aussi. pourquoi nous n'avions pas 40 ASes dans l'experience précedente + Mentionné que c'est une vision de Geant?}
