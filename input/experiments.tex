...
the competition has started. %%is already partially triggered and
propagating.
...
\begin{asparadesc}
  ...
  \item[Description:] We build a network of networks by using another
    ...
    \noindent We evaluate \NAME against an extension of \NAME called
    \NAMEC that enables lazy dynamic partitioning by allowing gateways
    to stop notifications when their respective network does not
    appear to have a source. Using \NAMEC, each \process maintains a
    \emph{local} index that corresponds to the best partition of its
    autonomous systems, and a \emph{global} index that corresponds to
    the best partition cross autonomous systems, \ie adjacent
    autonomous systems that have at least one source.
...
\begin{asparadesc}
  \item[Overall,] these experiments highlight the scalability of \NAME
    ...
    Section reviews state-of-the-art approaches designed to index
    content in geo-distributed infrastructures.
\end{asparadesc}
