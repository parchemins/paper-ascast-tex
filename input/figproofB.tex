...

  ...
  \draw[<->] (-5pt - \X, -\Y) -- node[above]{III} (5pt, -\Y); %% III    
  \draw[->] (-\X + 5pt, 0) -- node[above, font=\small, color=\PC]{$\DEL_d$} (0 - 5pt, 0); %% d - e

  \draw[<->] (-5pt , 5pt -\Y) -- node[above]{IV} (5pt + \X, 5pt -\Y); %% IV
  \draw[->] (0 +5pt, 0) -- (\X -5pt, 0); %% e - b'
  
  \draw[<->] (-5pt + \X , 10pt -\Y) -- node[above]{V} (5pt + 2*\X, 10pt -\Y); %% V
...

  ...
  \draw ( 0, 5pt) node[above, color=\WRONG]{$\ADD_f^{0.5}$};
