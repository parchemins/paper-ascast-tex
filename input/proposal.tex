...
exploit their local knowledge gathered over message receipts to allow
up-to-date messages to propagate to relevant \processes.


\begin{theorem}[\label{theo:dcp}BEF$^+$: BEF $\wedge$ (Purge $\rightarrow$ BEF) $\implies$
...
\begin{proof}
  ...
  \end{asparadesc}
  By using BEF along with purge of stale messages that triggers BEF,
  \processes reach consistent partitioning together as in
...
\begin{theorem}[\label{theo:threephase}Three-phase purge]
  ...
  that may have delivered it last requires three phases:
  \begin{inparaenum}[(i)]
...
\begin{proof}
  ...
  and forwarded a message $\alpha_a$. Some \processes may have
  delivered messages about other partitions before or after
  $\alpha_a$.
  \begin{asparadesc}
  \item [\processes with last $\alpha_a$:] \Processes such as
    \Process~$a$ where the last and best partition is still $P_a$. By
    propagating from \process to \process the corresponding $\delta_a$
    message, these \processes purge $\alpha_a$.
  \item [\processes with $\alpha_a^y \rightarrow \alpha_e^z$ with last
    $\alpha_e^z$, for $\alpha_e^z < \alpha_a^y$:] \Processes such as
    \Process~$e$ that do not directly suffer from the non-delivery of
    $\delta_a$. They need to purge their own $P_e$ if need
    be. Nonetheless, they deliver and forward $\alpha_e$ that at least
    one following \process will receive.
  \item [\processes with $\delta_e \rightarrow \alpha_d$:] \Processes
    such as \Process~$f$ that do not belong to the preceding category,
    for they already delivered $\delta_e$. Nevertheless, best eventual
    forwarding guarantees that they receive $\alpha_e$, which
    contradicts their history or state. In such a case, the detecting
    \processes purge their own $\alpha_a$. The detecting \processes
    ...
    consistency.  We note such a notification $\Delta_a$ to emphasize
    the uncertainty of a $\delta$ message.
  \item [\processes with last $\alpha_a$ receiving $\Delta_a$ from
    their parent:] \Processes such as \Process~$g$ that have
    $\alpha_a$ but preceding delivered messages are not
    important. Such \processes must trust detecting \processes by
    delivering and forwarding $\Delta_a$. It is worth noting that
