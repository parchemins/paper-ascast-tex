...
\begin{theorem}[\label{theo:bef}\underline{E}ventual \underline{F}orwarding
...
ensure consistent partitioning, eventual forwarding of best (see
...
\item eventual purging of stale messages ($S(m)
  ...
  d_q(m'))$);
\item then the eventual forwarding of best such that $m'$ ends up
...
\begin{proof}
  ...
  \item [Purge then EFB:] Assuming the eventual and definitive removal
    ...
    messages, or no message at all ($\eventually d_c(m') \implies \neg
    S(m')$).
...
\begin{proof}
  ...
  A \process (such as $d$) whose last delivery is $\alpha_X$, but
  whose parent is neither inconsistent (like $b$) nor receiving
