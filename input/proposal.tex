...
\begin{definition}[\label{def:echo}FB$^+$: echos] FB where a
...
\begin{lemma}[\label{def:fbfse}FB$^+ \wedge$ FS $\centernot\implies$ DCP]
...
\begin{lemma}[\label{def:detection}\underline{D}etector existence (D)]
...
\begin{proof}
  ...
  (A) Assuming a chain of delivery $C = [x, \ldots, z]$ of $\alpha_x$ with
  $\forall p \in C: \alpha_x = \min D_p$, and assuming
  ...
  \noindent Assuming a chain of delivery $C = [x, \ldots, z]$ of
  $\alpha_x$ with $\alpha_x = \min D_z$, and assuming $d_x(\delta_x)$,
  then by (A), the staleness notification reaches a \process $y\in C$ 

  
  every \process eventually deliver and forward $\delta_x$ either
  following the same chain, except if $\alpha_x$ is already stale
  which



  $z$ does not receive, hence deliver $\delta_x$ when a \process $y$
  between $x$ and $z$ stops forwarding $\delta_x$ because it delivered
  another message $\alpha_s$ with $\alpha_s < \alpha_x$.
...
blocked. \TODO{More about figure.} \TODO{Challenging?} 

\begin{definition}[FS$^+$: forwarding staleness downstream]
  To further reduce the scope  \TODO{Meow.}
\end{definition}

\begin{theorem}[$\textup{FB}^+ \wedge \textup{FS}^+ \wedge \textup{D} \implies \textup{DCP}$]
  \TODO{Miaw}
\end{theorem}
...
%% information.
%% One could guarantee consistent partitioning by always
