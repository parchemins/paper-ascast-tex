...
\begin{definition}[\underline{C}onsistent \underline{p}artitioning (CP)]
  Let $W_{xy} = W_{yx}$ be the positive \TODO{symmetric} \underline{w}eight
...
\begin{theorem}[\label{theo:efb}\underline{E}ventual
  \underline{F}orwarding of \underline{B}est $(EFB) \implies CP$]
  ...
  best known partition. \TODO{setminus parent to comply with implem'.}
\end{theorem}
...
\begin{definition}[\label{def:purge}Message purge]
  A system \underline{p}urges a message $\mathcal{P}(m)$ when every
