...
notifications.  This also applies to dynamic networks where nodes
join, leave, or crash at any time.
...
\begin{definition}[\label{def:forwarding}Forwarding]
  ...
  \underline{s}ends the latter ($s_{xy}(m)$) to all its neighbors $y$
  accumulating or aggregating ($\oplus$) metadata that depends on its
