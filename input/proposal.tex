...
different trade-offs on space, time, and communication. In this paper,
we introduce an adaptive scoped broadcast the cost of which actually
depends on its operations, and the current scopes that exist in the
system.
...
network state. For process to reach consistent partitioning together,
a process becoming a source must notify every process that is closer
to it than any other source. Epidemic
...
\begin{theorem}[\underline{B}est \underline{e}ventual \underline{d}elivery $(BED) \implies CP$]
  Assuming reliable communication links, every process eventually
  delivers, hence forwards, the notification message $\alpha_s$ of its
  best partition $P_s$, eventually leading to consistent partitioning.
\end{theorem}
