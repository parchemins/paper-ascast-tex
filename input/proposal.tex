...
\begin{theorem}[\label{theo:dcp}Purge + BEF $\implies$
...
\item eventual purging of stale messages ($S(m)
  ...
  d_q(m'))$);
\item and best eventual forwarding so such an $m'$ ends up originating from 
  the best up-to-date source for $q$.
\end{inparaenum}
\end{theorem}
...
\begin{proof}
  ...
  %% source: $d(m)$
  % As for CP, DCP requires that every process' last delivery 


  
  \processes deliver and forward the best sum of weights they
  received. Figure~\ref{fig:proofA} depicts the generic case where a
