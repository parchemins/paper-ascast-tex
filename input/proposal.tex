...
source, along with enough metadata to allow them to decide of their
closest source. A simple implementation consists in ensuring that each
\process $x$ eventually receives $\alpha_s^{|\Pi_{sx}|}$ where
$\Pi_{sx}$ is the \emph{best} path from any source to it; the value
$|\Pi_{sx}|$ being built over forwarding along this path.
...
\begin{theorem}[\label{theo:fb}\underline{F}orwarding of \underline{B}est (FB)
    ...
    links ($s_{xy}(m) \iff r_{yx}(m)$), \processes eventually reach
...
\begin{proof}
  ...
  % \noindent \TODO{use total order ($<$) and sets $R_p D_p$}
  Whenever a \process $s$ becomes a source, it delivers its own
  message $d_s(\alpha_{s}^{|\pi_{ss}|})$. Whatever its set of received
  messages, its own partition $P_s$ becomes and remains its closest
  partition $\alpha_{s}^{|\pi_{ss}|} = \min D_s$ since $|\pi_{ss}|$ is
...
\emph{reactive} forwarding of best to reach consistent partitioning
when
\begin{inparaenum}[(i)]
...
\end{inparaenum}
\TODO{We only keep last.}  For the sake of simplicity and to remove
obvious loops, a \process does not forward a notification to its
sender (Line~\ref{line:accumulator}).  Figure~\ref{fig:add}
...
\TODO{\underline{a}dd message}: $\alpha_a^0$ and $\alpha_d^0$. They
