...
%For instance, weights could be latency in live
...
\begin{theorem}[\underline{B}est \underline{e}ventual \underline{f}orwarding $(BEF) \implies CP$]
...
\begin{proof}
  ...
  transitively reaches all such \processes through their shortest
  paths: $\forall q'' \in V, s_1, s_2 \in S: |\Pi_{q'' s_1}| <
  |\Pi_{q'' s_2}| \implies \eventually d_{q''}(\alpha_{s_1}^{|\Pi_{s_1
      q''}|})$.  Since there exists only one best sum of weights per
  \process that can never be retracted, \processes eventually reach
...
\begin{theorem}[Propagation $+$ detection $\implies$ DCP]
  % Propagating 
...
comprises \smash{$\mathcal{O}(\frac{|V|}{2^{\lfloor \log_2 a
      \rfloor}})$} \processes. This decreases every new partition
