...
importantly between message deliveries. When a \process $s$ performs a
\texttt{Del} operation, this implicitly states that all its preceding
...
A naive implementation based on scoped broadcast as stated in
Definition~\ref{def:scoped} resembles echos in acoustics: staleness
notifications propagate as long as receivers' current partition is the
targeted one, and when staleness notifications reach the bordering
\processes of the deleted partition, it triggers a competition -- or
an echo -- that goes backward to fill the gap left open by removals.
...
\begin{definition}[\label{def:fbplus}$\textup{FB}^+$: echos] FB where a
...
Figure~\ref{fig:del} illustrates the behavior of this intuitive
implementation.  Two partitions initially exist: $P_a$ and $P_d$ that
respectively include $\{a\}$, and $\{b, c, d\}$. In
Figure~\ref{fig:delA}, $a$ deletes its partition. It notifies all
neighboring \processes~--~here only $b$~-- that may belong to its
partition. Upon receipt, $b$ discards the delete notification
$\delta_a$ since $\delta_a$ does not target the former's partition
$P_d$. \Process~$b$ sends its own best partition $\alpha_d^3$ that may
be the best for $a$. Eventually, every \process belongs to Partition
