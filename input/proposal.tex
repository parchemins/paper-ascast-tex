...
\begin{definition}[\label{def:echo}\underline{E}chos (E)]
...
\begin{lemma}[\label{def:fbfse}FB+FS+E $\centernot\implies$ DCP]
...
\begin{lemma}[\label{def:detection}Detector existence]
...
\TODO{But cannot just delete. It must be deleted only downstream, see
  figure.}
...
\begin{lemma}[FB $\wedge$ FD $\implies$ Detection] \TODO{meow!}
\end{lemma}
