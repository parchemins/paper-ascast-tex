...
\begin{definition}[\label{def:forwarding}Forwarding]
  ...
  \underline{s}ending it to its neighbors $y$ \TODO{enriched}
  ($\oplus$) with metadata that depends on its local knowledge
  $\sigma_x$: $f_x(m) \implies \forall \langle x, y\rangle \in E:
  s_{xy}(m \TODO{\oplus} \sigma_x)$.
\end{definition}
...
implementation based on Definition~\ref{def:scoped} and
Definition~\ref{def:forwarding} consists in ensuring that each
\process $x$ eventually receives $\alpha_s^{|\Pi_{sx}|}$ where
$\Pi_{sx}$ is the \emph{best} path from any source to it; the best
value $|\Pi_{sx}|$ being built over forwarding along this path.
...
blocking farther but up-to-date messages from actual sources.
...
reaching consistent partitioning. For instance, $c$ detects the
blocking of $\delta_a$ because it acknowledges that its parent $b$
delivered a stale notification $\alpha_c^1$, \ie between the delivery
of $\alpha_c^1$ and the delivery of $\delta_c$, $b$ blocked any other
$\delta$ and therefore $\delta_a$.
...
\begin{proof}
  ...
  $\alpha_x$ with $\forall y \in \pi_{xz}: \alpha_x = \last D_y$, and
  ...
  $\alpha_x = \last D_z$, and assuming $d_x(\delta_x)$, then by (A),
  ...
  \item
    ...
    \last D_{y'}$ which detects a possible blocking of
...
Figure~\ref{fig:problemD}, $c$ could broadcast $\delta_a$ in order to
acknowledge the staleness of its own $\alpha_a^3$ and inform
neighboring \processes that delivered it as well. Unfortunately, proof
of Lemma~\ref{lem:detector} shows that there exists false positive in
the blocking detection: $\delta_x$ may not exist but $y'$ still
receives $\alpha_s$ after the delivery of $\delta_s$ from the path it
delivered $\alpha_x$. Figure~\ref{fig:undoproblem} highlights this
behavior. In Figure~\ref{fig:undoproblemB}, $a$ does not delete its
partition. In Figure~\ref{fig:undoproblemD} and because of scoped
broadcast, $c$ received the same series of messages as in
...
notifications by including \TODO{only downstream \processes}. In
...
enables dynamic consistent partitioning in dynamic networks subject to
physical partitioning where \processes can join, leave, or crash at
any time. Adding a \process is equivalent to add as many edges as
necessary. Since adding an edge may improve shortest paths, the
protocol triggers a competition using echos of
Definition~\ref{def:fbplus}.  Removing a \process is equivalent to
removing all its edges. Removing an edge between two \processes may
invalidate the shortest path of one of involved \processes plus
downstream \processes, or impair the propagation of staleness
notifications. Therefore, the protocol reuses its detectors of
Lemma~\ref{lem:detector} to remove irrelevant messages from the
system.
