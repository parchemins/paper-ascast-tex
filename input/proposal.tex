...
\begin{theorem}[\label{theo:efb}\underline{F}orwarding of \underline{B}est (FB)
  ...
  %
  Assuming reliable communication links ($s_{pq}(m) \iff \eventually
  r_{qp}(m)$), \processes eventually reach consistent partitioning if
  each \process delivers and forwards its best message among
  \underline{r}eceived messages $R_p$, \ie the message $\alpha_s^d$
  with the smallest distance $d$ to the source $s$ ($(\alpha_s^d = \min R_p)
  \implies (d_p(\alpha_s^d) \wedge f_p(\TODO{\alpha_s^d}))$).
\end{theorem}
...
\begin{proof}
  ...
  \in E \iff \eventually r_q(\alpha_{s}^{w_{pq}})$. Most importantly,
...
\item weights are scalar values piggybacked in messages and
  accumulated over forwardings,
\item \processes only add new partitions to the system,
\item and \processes never join, crash nor leave the system.
\end{inparaenum}
For the sake of simplicity, a \process does not forward a notification
to its sender (Line~\ref{line:accumulator}).  Figure~\ref{fig:add}
illustrates its behavior on a system comprising 4 \processes $a$, $b$,
$c$, and $d$. \Process~$a$ and \Process~$d$ become the sources of
