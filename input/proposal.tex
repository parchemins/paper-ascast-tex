...
To reach consistent partitioning, \processes need
\begin{inparaenum}[(i)]
...
\item enabling up-to-date notifications to eventually echo back to
  every \process.
\end{inparaenum}
Since Figure~\ref{fig:problemD} highlights that when the protocol
terminates, $c$ is the \process that remains in the wrong partition,
we put the burden of detection and resolution on it. 
...
\begin{lemma}[\label{lem:detector}$\textup{FB}^+ \wedge \textup{FS}
  ...
  $\alpha_x$ with $\alpha_x = \textup{last}~D_z$, if $x$ broadcasts a staleness
