...
\begin{theorem}[\label{theo:dcp}BEF $\wedge$ (Purge $\rightarrow$ BEF) $\implies$
...
\begin{proof}
  ...
  \item [Purge not followed by BEF:]
    ...
    Assuming that every process removes control information as soon as
    they become stale ($S(\alpha_a^x) \implies d_b(\alpha_a^x)
    ...
    consistent partitioning.
  \item [Purge then BEF:] Assuming the eventual removal of stale
    control information , 


    nothing stops up-to-date control information to propagate, as in
    Theorem~\ref{theo:bef}. Assuming that removal of stale control
    information immediatly triggers a forwarding of best up-to-date
...
\begin{figure}
  ...
  \caption{\label{fig:proofB}Stale $\alpha$ messages may
      ...
      when parents' $\alpha$ messages contradict history or state (III); notify
