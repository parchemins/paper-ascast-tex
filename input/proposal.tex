...
\begin{proof}
  ...
  $\alpha_X$.
  \begin{asparadesc}
  \item [\processes with last $\alpha_X$:] \Processes such as
    \Process~$a$ where the last and best partition is still $P_X$. By
    propagating from \process to \process the corresponding $\delta_X$
    message, these \processes purge $\alpha_X$.
  ...
  \item [\processes with $\delta_Y \rightarrow \alpha_X$:] \Processes
    ...
    their relative similarity to $\delta_X$ messages.
...
\end{inparaenum}
% This vector enables the simplest Propagation~I and Propagation~I'.
Its size increases linearly with the number of sources that the
\process has ever known, which is the number of \processes in the
system $\mathcal{O}(V)$ in the worst case.
...
$\Delta_{s, c}^{\pi}$ fulfils the double role of removing stale
...
reflect such a delivery, messages piggyback monotonically such
increasing control information).

$\delta_{s, c}$ messages used in Propagation~I are more efficient than
$\Delta_{s, c}^\pi$ messages used in Propagation~III, both in
...
increases.
%% Bloom filters could introduce another trade-off by further
