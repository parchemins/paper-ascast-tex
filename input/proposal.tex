...
\begin{theorem}[Dynamic consistent partitioning]
...
\begin{inparaenum}[(i)]
\item most of the time, it propagates $\alpha$'s and $\delta$'s when
  the partitions allow it;
\item sometimes, it detects possible inconsistent partitioning and
\item solves it using propagation trees (as opposed to propagation
  graphs);
\item when required, it triggers competitions among neighboring
  partitions.
\end{inparaenum}
...
\noindent To implement last win order, each process maintains a vector
of versions that associates to each known source, or has-been source,
its known local counter. It enables processes to deliver only
up-to-date $\alpha$'s that may improve their best known partition (see
...
\noindent Upon possible inconsistency, the detecting process must
remove all $\alpha$ messages originating from it, as labeled by IV in
Figure~\ref{fig:proofB}. Contrarily to delete operations, this applies
to a sub-partition of the current partition, and cannot rely on local
counters to operate. Therefore, we implement this behavior by
piggybacking paths in messages.  \TODO{Paths and \NAME synergies well,
  for paths tend to be smaller as the number of sources increase in
  the system.}
