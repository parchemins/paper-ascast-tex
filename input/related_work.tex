...
location services host by possibly remote third-party \processes; and
...
\noindent Such solutions inherently imply that every \process receives
every message, which contradicts with the principles of Edge
infrastructures that aim at reducing the volume of data passing
through the network. On the opposite, each \process running \NAME only
registers its closest replica. This allows \NAME to use scoped
broadcast as a communication primitive to lock down the traffic
generated by content indexing based on distances.
% Such solutions inherently imply that every node will
% eventually receive all messages, contrary to \NAME.

\item [Scoped flooding:]

Some state-of-the-art approaches also confine the dissemination of
information to interested \processes based on distance. 
...
\end{inparaenum}
On the opposite, \NAME quickly inform each \process of their closest
