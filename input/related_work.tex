...
\item request to possibly far away location services the content
  location (e.g. as in \underline{c}ontent \underline{d}elivery
  \underline{n}etworks (CDN)~\cite{triukose2011measuring} based on
  \underline{d}omain \underline{n}ame \underline{s}ystems (DNS)), and
  then
\item request the actual content from possibly far away replicas
  (e.g. as with \underline{d}istributed \underline{h}ash
  \underline{t}ables (DHT)~\cite{ipfs, mdht, squirrel}).
\end{inparaenum}
For instance, using DHTs~\cite{ipfs, mdht, squirrel}, each \process
...
replica even in large networks; % when the system is quiescent,
