...
\begin{tikzpicture}[scale=0.87]
  ...
  (0, 0); %% a - b
  \draw (0, 0) --
  node[above, font=\tiny]{$\ADD_c^1 \DEL_c$}
  node[below, font=\tiny]{$\ADD_a^{2.5}$} (\X, 0); %% b - c
  \draw (0, 0) --
  node[above, font=\tiny, sloped]{$\ADD_a^{2.5}$}
  (0, \Y); %% b - d
  \draw (0, \Y) --
  node[above, font=\tiny, sloped]{$\ADD_c^2$}
  (0, 0); %% d - b

  \draw (\X, 0) --
  node[above, font=\tiny, sloped]{$\ADD_c^2$}
  node[below, font=\tiny, sloped]{$\DEL_c$}
  (0, \Y); %% c - d
