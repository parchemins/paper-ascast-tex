...
\Function{\textup{\texttt{Add ( )}}} { % \hfill [if $V[p]\equiv 0 \mod 2 $]} } {
  \textup{\smash{receiveAdd($p$, $\alpha_{[\langle p, V[p] + 1\rangle]}^{0}$)}}\label{line:ascast_add}
}
...
\Function{\textup{receiveAdd($q$, $\alpha_{\pi'}^{d'}$)}} {
  \uIf{$\textup{isParent}(q) \algoAnd \textup{isStale}(\alpha_{\pi'}^{d'})$ \label{line:ascast_version}} {
    \textup{receiveDel($q$, $\delta_{p, V[p] + 1}$)} \tcp*[h]{\footnotesize{(II) send $\Delta$}}
  }
  \uElseIf {\footnotesize$\alpha_{\pi'}^{d'} < A_{\pi}^{d} \algoAnd \neg\textup{isStale}(\alpha_{\pi'}^{d'}) \algoAnd \neg\textup{isLoop}(\alpha_{\pi'}^d)$  } {
    $V[s'] \leftarrow c'$ \;
    $\smash{A_{s, c}^{d, \pi} \leftarrow \alpha_{s', c'}^{d', \pi'}}$ \;
    \ForEach(\tcp*[h]{\footnotesize{forward}}){$n \in O_p$} {
      send$_n$(\smash{$\alpha_{s', c'}^{d' + W_{pq}, \pi' \cup p}$})\label{line:ascast_better}
    }
...
\lFunction{\textup{isStale}(\smash{$\alpha_{\pi'}^{d'}$})} {
  \Return \smash{$\exists \langle q, c \rangle \in \pi': c < V[q]$}
}
