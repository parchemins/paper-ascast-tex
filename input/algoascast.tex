...
\Function{\textup{receiveAdd($q$, $\alpha_{\pi'}^{d'}$)}} {
  \uIf {$\alpha_{\pi'}^{d'} < A_{\pi}^{d} \algoAnd \neg\textup{isStale}(\alpha_{\pi'}^{d'}) \algoAnd \neg\textup{isLoop}(\alpha_{\pi'}^{d'})$  } {
    $\smash{A_{\pi}^{d} \leftarrow \alpha_{\pi' \cup \langle p, V[p] \rangle}^{d'}}$ \;
    \lForEach{$n \in O_p$} {
      send$_n$(\smash{$\alpha_{\pi}^{d + W_{pn}}$})\label{line:ascast_better}
    }
  }
  \uElseIf{$\textup{isParent}(q) \algoAnd \textup{isStale}(\alpha_{\pi'}^{d'})$ \label{line:ascast_version}} {
...
\Function{\textup{receiveDel($q$, $\delta_{s, c}$)}} {
  \uIf {$\exists \langle s, c' \rangle \in \pi: c' < c$} {
    $A^d_\pi \leftarrow \alpha^\infty_\varnothing$ \;
