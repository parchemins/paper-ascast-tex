...
\begin{tikzpicture}[scale=0.87]
  ...
  \draw[opacity=0] (-1.4*\X, 0) -- (1.4*\X, 0);
  


  \draw[<->] (-5pt - \X, -\Y) -- node[above]{I: propagation} (5pt, -\Y); %% I
  
  \draw[->] (-\X + 5pt, 0) --
  node[above, color=\PA, font=\small]{$\ADD_a^1$}
  (0 - 5pt, 0); %% a - b

  \draw[<->] (-5pt, 5pt -\Y) -- node[above]{II: need I', III, IV} (5pt + \X, 5pt -\Y); %% II
...

  ...
  \draw (0, -5pt) node[below, font=\scriptsize]{expect $\DEL_d \rightarrow \textcolor{\PA}{\alpha_a}$};
